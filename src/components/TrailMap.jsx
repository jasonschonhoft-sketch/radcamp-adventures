...
function TrailLayer({ activeFilters, singletrackOnly, onStatusChange, routeMode, onAddToRoute, routeTrails }) {
  ...
  const loadTrails = useCallback(async () => {
    ...
    try {
      ...
      // Re-highlight route trails after map move
      if (routeTrailsRef.current?.length > 0) {
        const routeNames = new Set(routeTrailsRef.current.map(t => t.name));
        ACTIVITY_KEYS.forEach(activity => {
          (polylinesRef.current[activity] || []).forEach(p => {
            const name = p.__properties?.name?.trim();
            if (name && routeNames.has(name)) {
              p.setOptions({ strokeColor: '#ffffff', strokeWeight: (p.__origWeight || 2) + 3, strokeOpacity: 1 });
              routePolylinesRef.current.push({ polyline: p, origColor: p.__color, origWeight: p.__origWeight, origOpacity: p.__origOpacity });
            }
          });
        });
      }
